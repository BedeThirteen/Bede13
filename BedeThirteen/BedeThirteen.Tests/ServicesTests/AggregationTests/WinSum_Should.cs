    ...
    public class WinSum_Should
    {
        ...
        public async Task ReturnSum_WhenInput_Isvalid(int numberOfTransactions)
        {
            ...
            var options = new DbContextOptionsBuilder<BedeThirteenContext>()
                .UseInMemoryDatabase($"ReturnWinSum_WhenInput_Isvalid_{numberOfTransactions}").Options;
            var stakeTransactionType = new TransactionType() { Name = "Win", };
            ...
            using (var context = new BedeThirteenContext(options))
            {
                ...
                result = await sut.WinsSum(DateTime.MinValue, DateTime.MaxValue);
            }
        ...
        public async Task ReturnStakeOfUser_WhenId_IsGiven()
        {
            // Arrange
            var options = new DbContextOptionsBuilder<BedeThirteenContext>()
                .UseInMemoryDatabase("ReturnWinSum_WhenInput_Isvalid").Options;
            ...
            using (var context = new BedeThirteenContext(options))
            {
                ...
                // Act
                var result = await sut.WinsSum(DateTime.MinValue, DateTime.MaxValue, mockUser.Id);
        ...
        public async Task ReturnZero_TimeInterval_IsEmpty()
        {
            // Arrange
            var options = new DbContextOptionsBuilder<BedeThirteenContext>()
                .UseInMemoryDatabase("ReturnZero_TimeInterval_IsEmpty").Options;
            ...
            using (var context = new BedeThirteenContext(options))
            {
                ...
                // Act
                var result = await sut.WinsSum(DateTime.Now, DateTime.MaxValue);
